...
Tester::Tester(const std::vector<std::vector<int>> &adj_list, const std::vector<int> &ground_truth_sizes,
        ...
        initialization_style_(std::move(initialization_style)) {
    num_augmentations = 0;
...
double Tester::RunTest(int k) {
    ...
    GraphicMatroid graph(adj_list_, k, initialization_style_);
...
void Tester::RunWithoutChecking(int k) {
    // runs a test, updates statistics
    ...
    auto forests = graph.GetForests();
    optimal_size = SumOfSizes(forests);

    num_augmentations = graph.num_augmentations;
    shortest_augmentation_length = graph.shortest_augmentation_length;
    if (shortest_augmentation_length == INT32_MAX) {
        shortest_augmentation_length = 0;
    }
    longest_augmentation_length = graph.longest_augmentation_length;
    num_find_edge_levels = graph.num_find_edge_levels;
    num_find_edge_bfi = graph.num_find_edge_bfi;
}
